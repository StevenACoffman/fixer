...
// This file defines the linter that kacontext interfaces aren't unnecessarily
// large, and that any interface-component that's used explicitly is mentioned
// explicitly.
//
...
//   (or, as a special case, if J is kacontext.Base, it's fine for v to just
//   use context.Context)
...
var KAContextInterfaceAnalyzer = &analysis.Analyzer{
	Name: "kacontextinterface",
	Doc:  "enforces that kacontext interfaces aren't unnecessarily large",
	Run:  _runInterface,
}

// _explicitInterfaces returns the KAContext-style interfaces explicitly
...
//	_leafInterfaces(C) => C
//
// TODO(benkraft): Stopping at interfaces with methods is sort of a heuristic.
...
// either A or `{ B; M() }`.  Luckily, the only place we do this is in pkg with
// kacontext.Base (e.g. pkg/gcloud/pubsub.LogErrorAsync); callers outside of
// pkg don't really care (even when calling such a function, since we
// special-case that if you ask for kacontext.Base and only use
// context.Context, that's ok).  One way to solve for this would be to have
// some base interface included in each context, but that would require adding
// new packages (pkg/kacontext is too high), and doesn't seem to have many
// benefits other than in this linter.
...
//
// TODO(benkraft): If other callers need this, move it to dev/lintutil.
...
// unqualified.
//
// TODO(benkraft): If other callers need this, move it to lintutil.
func _shortTypeName(typ types.Type, pkg *types.Package) string {
	...
	if typ, ok := typ.(*types.Named); ok {
		...
		// TODO(benkraft): If we ever need it, abbreviate interface/struct
		// names by writing the method/field names, omitting signatures/types.
	}
...
// _maybeNeededForCache returns true if this type might be needed to satisfy
// a cache decorator (e.g. memorystore.KAContext, if you used
// memorystore.Cache).
//
// TODO(benkraft): Actually figure out which types this particular cached
// function needs.  For now, we just allow anything that any cache needs.
func _maybeNeededForCache(typ types.Type) bool {
	// used by settingscache
	return lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/kacontext", "Base") ||
		// used by datastore, settingscache, and Expiration
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/lib/timectx", "KAContext") ||
		// used by memorystore and settingscache
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/lib/log", "KAContext") ||
		// used by memorystore and settingscache
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/gcloud/memorystore", "KAContext") ||
		// used by datastore and settingscache
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/gcloud/datastore", "KAContext") ||
		// used by PersistAcrossPublish
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/content", "KAContext") ||
		// common in key-params-fxns (or perhaps a future cache-option!)
		//
		// TODO(benkraft): Having a key-params-fxn ask for a context that
		// the cached function doesn't use suggests that we are
		// over-caching.  The problem is that the GraphQL client will
		// forward these headers even if they aren't in your context.
		// That's a recipe for caching bugs!
		//
		// Instead, we should do something like: have the GraphQL client
		// ask for ka-locale-context if and only if we are forwarding the
		// ka-locale header (and so on for other contexts).  It's unclear
		// if there's a reasonable way to do that without generics, though.
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/web", "CountryContext") ||
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/web", "CurriculumContext") ||
		lintutil.TypeIs(typ, "github.com/Khan/webapp/pkg/web", "KALocaleContext")
}
...
func (tracker *_interfaceTracker) track(ident *ast.Ident) {
	...
	// Tests can ask for TestContext, which is useful if you want to Clone(),
	// or just if you want to make it obvious that you are a test util and
	// tests should pass in their `suite.KAContext()`.  In any case, we'll
	// allow it.
	if lintutil.TypeIs(obj.Type(), kacontextPath, "TestContext") {
		return
	}
...
//
// For example, if you call secrets.Validate(ctx), this will mark the
// secrets.KAContext interface of ctx as used.
...
func (tracker *_interfaceTracker) markUses(startNode ast.Node) {
	ast.Inspect(startNode, func(node ast.Node) bool {
		...
		case *ast.CompositeLit: // struct, map, or array
			...
			// use it in a struct literal, etc.  Luckily, these don't come
			// up much for kacontext, so we don't worry about them yet.
		}

		return true // otherwise, recurse
	})
	// This is a separate traversal, because it needs two passes and needs to
	// happen last:
	tracker.markTracegroupUses(startNode)
}

// markTracegroupUses marks places where a context is used via
// pkg/gcloud/stackriver/tracegroup.
//
// Specifically, calls to tracegroup look like
//	func MyFunc(ctx firstContext, ...) error {
//		g := tracegroup.WithContext(&ctx)
//		g.Go(func(ktx secondContext) error {  // (several of these)
//			...
//		}
//		return g.Wait()
//	}
// In this case, the fact that the callback-function accepts secondContext
// should count as a use of that type for ctx (indeed the tracegroup runtime
// validation ensures that secondContext is a subset of firstContext).  This
// function adds such uses to the tracker.
func (tracker *_interfaceTracker) markTracegroupUses(startNode ast.Node) {
	groupToCtx := map[types.Object]types.Object{}

	// First, look for
	//	g := tracegroup.WithContext(&ctx)
	// We don't look super hard for nonsense like
	//  myStruct.groups[0], unrelated := tracegroup.WithContext(&ctx), 123
	// since it shouldn't be very common and is hard to get right.
	ast.Inspect(startNode, func(node ast.Node) bool {
		...
		funcName := lintutil.NameOf(lintutil.ObjectFor(call.Fun, tracker.typesInfo))
		if funcName != "github.com/Khan/webapp/pkg/external/opentelemetry/tracegroup.WithContext" {
			return true
		}
		addrExpr, ok := call.Args[0].(*ast.UnaryExpr)
		if !ok || addrExpr.Op != token.AND {
			return true
		}
		ctxObj := lintutil.ObjectFor(addrExpr.X, tracker.typesInfo)
		ctxInfo := tracker.trackedIdents[ctxObj]
		if ctxObj == nil || ctxInfo == nil {
			return true
		}
		groupObj := lintutil.ObjectFor(assign.Lhs[0], tracker.typesInfo)
		if groupObj == nil {
			return true
		}

		// Track this mapping for below.
		groupToCtx[groupObj] = ctxObj

		// Also count this as a use of context.Context on the group-context
		// (since it is).
		for _, iface := range _leafInterfaces(ctxInfo.obj.Type()) {
			if lintutil.TypeIs(iface, "context", "Context") ||
				lintutil.TypeIs(iface, kacontextPath, "Base") {
				ctxInfo.interfaceUses[iface] = true
			}
		}
	...
	// Now, look for
	//	g.Go("...", func(ctx ...) error { ... })
	// Again we don't look for if you, say, pass a value of type interface{}
	// which is defined elsewhere to be a function.
	ast.Inspect(startNode, func(node ast.Node) bool {
		call, ok := node.(*ast.CallExpr)
		if !ok || len(call.Args) != 2 { // spanName, f
			return true // recurse
		}
		...
		groupObj := lintutil.ObjectFor(sel.X, tracker.typesInfo)
		if groupObj == nil {
			return true
		}
		groupCtxObj, ok := groupToCtx[groupObj]
		if !ok {
			return true
		}
		groupCtxInfo := tracker.trackedIdents[groupCtxObj]
		if groupCtxInfo == nil {
			return true
		}

		funcType, ok := tracker.typesInfo.TypeOf(call.Args[1]).(*types.Signature)
		if !ok || funcType.Params() == nil || funcType.Params().Len() == 0 {
			return true
		}
		funcCtxType := funcType.Params().At(0).Type()

		// Phew!  Now, mark that the group-ctx was used as the type of the
		// func-ctx.  (If the func-ctx itself is not properly used, we'll
		// report that separately as usual.)
		groupCtxInfo.interfaceUses[funcCtxType] = true
...
// kacontext interfaces than you really wanted.  For example, if you have
...
//
// TODO(benkraft): We might also wish to check for the case where the interface
...
//
// TODO(benkraft): Another thing we should check with interfaces is that the
...
type _objInfo struct {
	...
	// some kacontext-interface.  (Specifically it contains the interface types
...
func (info *_objInfo) _interfaceWasUsed(typ types.Type) bool {
	// If the function is cached, we may have needed the interface for that.
	// (This is fast so we put it first.)
	if info.isCached && _maybeNeededForCache(typ) {
		return true
	}
	...
	// If you requested kacontext.Base, it's okay if you only used
	// context.Context.
	if lintutil.TypeIs(typ, kacontextPath, "Base") {
		context := _embedNamed(typ, "context", "Context")
		if context != nil && info._interfaceWasUsed(context) {
			return true
		}
	}
...
func (info *_objInfo) _interfaceWasRequested(typ types.Type) bool {
	...
	// we just give up on this check.  Usually that only happens in pkg (e.g.
	// in packages below kacontext), where people are mostly not using
	// multi-interfaces anyway so this whole method is less important.
	...
	for _, embed := range _explicitInterfaces(info.obj.Type(), info.obj.Pkg()) {
		...
		// If we used context.Context, it's ok if we requested kacontext.Base.
		if lintutil.TypeIs(embed, kacontextPath, "Base") &&
			lintutil.TypeIs(typ, "context", "Context") {
			return true
		}
	}
...
// _runInterface lints that you don't ask for kacontext interfaces you don't
// need.
//
